export const IndustryCarousel = () => {
  return (
    <div className="relative box-border caret-transparent block z-[1] overflow-hidden rounded-lg">
      <div className="relative caret-transparent h-full list-none w-full z-[1] overflow-hidden mx-auto">
        <div className="relative caret-transparent flex h-full w-full z-[1]">
          <div
            role="group"
            aria-label="1 / 5"
            className="relative box-border caret-transparent block shrink-0 h-full w-[343px] md:w-[720px]"
          >
            <video
              playsInline
              preload="auto"
              poster="https://c.animaapp.com/mkh4hbwu1IS6us/assets/home-industry-finserve.webp"
              muted={true}
              className="box-border caret-transparent max-w-full"
            >
              <source
                src="/video/home-industry-finserve.mp4"
                type="video/mp4"
                className="text-black box-border caret-transparent leading-[normal] list-disc font-times_new_roman"
              />
            </video>
            <a
              href="/industry/travel/"
              className="static items-center bg-[oklch(0.8346_0.0735_257.42)] box-border caret-transparent gap-x-3 flex shrink-0 justify-between leading-4 gap-y-3 w-fit ml-auto mt-4 pl-5 pr-1 py-1 rounded-[3.35544e+07px] right-8 bottom-6 md:absolute"
            >
              <span className="box-border caret-transparent block">
                AI CX for financial services
              </span>
              <span className="items-center aspect-square bg-[oklch(0.1489_0.0027_248.08)] box-border caret-transparent flex shrink-0 justify-center w-10 rounded-[3.35544e+07px]">
                <img
                  src="https://c.animaapp.com/mkh4hbwu1IS6us/assets/icon-4.svg"
                  alt="Icon"
                  className="box-border caret-transparent w-4"
                />
              </span>
            </a>
          </div>
          <div
            role="group"
            aria-label="2 / 5"
            className="relative box-border caret-transparent block shrink-0 h-full w-[343px] md:w-[720px]"
          >
            <video
              playsInline
              preload="auto"
              poster="https://c.animaapp.com/mkh4hbwu1IS6us/assets/home-industry-gaming.webp"
              muted={true}
              className="box-border caret-transparent max-w-full"
            >
              <source
                src="/video/home-industry-gaming.mp4"
                type="video/mp4"
                className="text-black box-border caret-transparent leading-[normal] list-disc font-times_new_roman"
              />
            </video>
            <a
              href="/industry/gaming/"
              className="static items-center bg-[oklch(0.8346_0.0735_257.42)] box-border caret-transparent gap-x-3 flex shrink-0 justify-between leading-4 gap-y-3 w-fit ml-auto mt-4 pl-5 pr-1 py-1 rounded-[3.35544e+07px] right-8 bottom-6 md:absolute"
            >
              <span className="box-border caret-transparent block">
                AI CX for gaming
              </span>
              <span className="items-center aspect-square bg-[oklch(0.1489_0.0027_248.08)] box-border caret-transparent flex shrink-0 justify-center w-10 rounded-[3.35544e+07px]">
                <img
                  src="https://c.animaapp.com/mkh4hbwu1IS6us/assets/icon-4.svg"
                  alt="Icon"
                  className="box-border caret-transparent w-4"
                />
              </span>
            </a>
          </div>
          <div
            role="group"
            aria-label="3 / 5"
            className="relative box-border caret-transparent block shrink-0 h-full w-[343px] md:w-[720px]"
          >
            <video
              playsInline
              preload="auto"
              poster="https://c.animaapp.com/mkh4hbwu1IS6us/assets/home-industry-retail.webp"
              muted={true}
              className="box-border caret-transparent max-w-full"
            >
              <source
                src="/video/home-industry-retail.mp4"
                type="video/mp4"
                className="text-black box-border caret-transparent leading-[normal] list-disc font-times_new_roman"
              />
            </video>
            <a
              href="/industry/ecommerce/"
              className="static items-center bg-[oklch(0.8346_0.0735_257.42)] box-border caret-transparent gap-x-3 flex shrink-0 justify-between leading-4 gap-y-3 w-fit ml-auto mt-4 pl-5 pr-1 py-1 rounded-[3.35544e+07px] right-8 bottom-6 md:absolute"
            >
              <span className="box-border caret-transparent block">
                AI CX for retail
              </span>
              <span className="items-center aspect-square bg-[oklch(0.1489_0.0027_248.08)] box-border caret-transparent flex shrink-0 justify-center w-10 rounded-[3.35544e+07px]">
                <img
                  src="https://c.animaapp.com/mkh4hbwu1IS6us/assets/icon-4.svg"
                  alt="Icon"
                  className="box-border caret-transparent w-4"
                />
              </span>
            </a>
          </div>
          <div
            role="group"
            aria-label="4 / 5"
            className="relative box-border caret-transparent block shrink-0 h-full w-[343px] md:w-[720px]"
          >
            <video
              playsInline
              preload="auto"
              poster="https://c.animaapp.com/mkh4hbwu1IS6us/assets/home-industry-saas.webp"
              muted={true}
              className="box-border caret-transparent max-w-full"
            >
              <source
                src="/video/home-industry-saas.mp4"
                type="video/mp4"
                className="text-black box-border caret-transparent leading-[normal] list-disc font-times_new_roman"
              />
            </video>
            <a
              href="/industry/saas/"
              className="static items-center bg-[oklch(0.8346_0.0735_257.42)] box-border caret-transparent gap-x-3 flex shrink-0 justify-between leading-4 gap-y-3 w-fit ml-auto mt-4 pl-5 pr-1 py-1 rounded-[3.35544e+07px] right-8 bottom-6 md:absolute"
            >
              <span className="box-border caret-transparent block">
                AI CX for technology
              </span>
              <span className="items-center aspect-square bg-[oklch(0.1489_0.0027_248.08)] box-border caret-transparent flex shrink-0 justify-center w-10 rounded-[3.35544e+07px]">
                <img
                  src="https://c.animaapp.com/mkh4hbwu1IS6us/assets/icon-4.svg"
                  alt="Icon"
                  className="box-border caret-transparent w-4"
                />
              </span>
            </a>
          </div>
          <div
            role="group"
            aria-label="5 / 5"
            className="relative box-border caret-transparent block shrink-0 h-full w-[343px] md:w-[720px]"
          >
            <video
              playsInline
              preload="auto"
              poster="https://c.animaapp.com/mkh4hbwu1IS6us/assets/home-industry-travel.webp"
              muted={true}
              className="box-border caret-transparent max-w-full"
            >
              <source
                src="/video/home-industry-travel.mp4"
                type="video/mp4"
                className="text-black box-border caret-transparent leading-[normal] list-disc font-times_new_roman"
              />
            </video>
            <a
              href="/industry/travel/"
              className="static items-center bg-[oklch(0.8346_0.0735_257.42)] box-border caret-transparent gap-x-3 flex shrink-0 justify-between leading-4 gap-y-3 w-fit ml-auto mt-4 pl-5 pr-1 py-1 rounded-[3.35544e+07px] right-8 bottom-6 md:absolute"
            >
              <span className="box-border caret-transparent block">
                AI CX for travel
              </span>
              <span className="items-center aspect-square bg-[oklch(0.1489_0.0027_248.08)] box-border caret-transparent flex shrink-0 justify-center w-10 rounded-[3.35544e+07px]">
                <img
                  src="https://c.animaapp.com/mkh4hbwu1IS6us/assets/icon-4.svg"
                  alt="Icon"
                  className="box-border caret-transparent w-4"
                />
              </span>
            </a>
          </div>
        </div>
      </div>
    </div>
  );
};
